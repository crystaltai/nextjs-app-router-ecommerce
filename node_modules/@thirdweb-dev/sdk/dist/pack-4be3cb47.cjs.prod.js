...
var contractPublisher = require('./contract-publisher-b0f742d6.cjs.prod.js');
var erc1155Standard = require('./erc-1155-standard-2b9cb4f4.cjs.prod.js');
...
var zod = require('zod');
var hasERC20Allowance = require('./hasERC20Allowance-b5de3858.cjs.prod.js');
...
const CommonWrappableSchema = /* @__PURE__ */zod.z.object({
  contractAddress: contractPublisher.AddressOrEnsSchema
});
...
const ERC721WrappableSchema = /* @__PURE__ */CommonWrappableSchema.extend({
  tokenId: contractPublisher.BigNumberishSchema
});
...
const ERC1155WrappableSchema = /* @__PURE__ */CommonWrappableSchema.extend({
  tokenId: contractPublisher.BigNumberishSchema,
  quantity: contractPublisher.BigNumberishSchema
});
...
const ERC1155RewardSchema = /* @__PURE__ */ERC1155WrappableSchema.omit({
  ...
  quantityPerReward: contractPublisher.BigNumberishSchema
});
...
const ERC20RewardContentsSchema = /* @__PURE__ */(() => ERC20RewardSchema.extend({
  totalRewards: contractPublisher.BigNumberishSchema.default("1")
}))();
...
const PackMetadataInputSchema = /* @__PURE__ */(() => PackRewardsOutputSchema.extend({
  ...
  rewardsPerPack: contractPublisher.BigNumberishSchema.default("1"),
  openStartTime: contractPublisher.RawDateSchema.default(new Date())
}))();
...
class PackVRF {
  featureName = contractPublisher.FEATURE_PACK_VRF.name;
  constructor(network, address, storage, options, chainId) {
    let contractWrapper = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : new contractPublisher.ContractWrapper(network, address, IPackVRFAbi__default["default"], options, storage);
   ...
   * @twfeature PackVRF
   */
  open = /* @__PURE__ */contractPublisher.buildTransactionFunction((() => {
    var _this = this;
   ...
   * @twfeature PackVRF
   */
  claimRewards = /* @__PURE__ */contractPublisher.buildTransactionFunction((() => {
    var _this2 = this;
  ...
  async parseRewards(rewards) {
    ...
    for (const reward of rewards) {
      switch (reward.tokenType) {
        case 0:
          {
            const tokenMetadata = await contractPublisher.fetchCurrencyMetadata(this.contractWrapper.getProvider(), reward.assetContract);
            erc20Rewards.push({
  ...
  async canClaimRewards(claimerAddress) {
    const address = await contractPublisher.resolveAddress(claimerAddress || (await this.contractWrapper.getSignerAddress()));
  ...
  getLinkContract() {
    const linkAddress = contractPublisher.LINK_TOKEN_ADDRESS[this.chainId];
    if (!linkAddress) {
      throw new Error(`No LINK token address found for chainId ${this.chainId}`);
    }
    const contract = new contractPublisher.ContractWrapper(this.contractWrapper.getSignerOrProvider(), linkAddress, ERC20Abi__default["default"], this.contractWrapper.options, this.storage);
    return new contractPublisher.Erc20(contract, this.storage, this.chainId);
  }
}
...
class Pack extends erc1155Standard.StandardErc1155 {
  static contractRoles = contractPublisher.PACK_CONTRACT_ROLES;
   ...
   */
  get vrf() {
    return contractPublisher.assertEnabled(this._vrf, contractPublisher.FEATURE_PACK_VRF);
  }
  constructor(network, address, storage) {
    let options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};
    let abi = arguments.length > 4 ? arguments[4] : undefined;
    let chainId = arguments.length > 5 ? arguments[5] : undefined;
    let contractWrapper = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : new contractPublisher.ContractWrapper(network, address, abi, options.gasless && "openzeppelin" in options.gasless ? {
      ...options,
      gasless: {
        ...options.gasless,
        openzeppelin: {
    ...
    this.metadata = new contractPublisher.ContractMetadata(this.contractWrapper, contractPublisher.PackContractSchema, this.storage);
    this.app = new contractPublisher.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new contractPublisher.ContractRoles(this.contractWrapper, Pack.contractRoles);
  ...
  async isTransferRestricted() {
    const anyoneCanTransfer = await this.contractWrapper.readContract.hasRole(contractPublisher.getRoleHash("transfer"), ethers.constants.AddressZero);
   ...
   * ```
   */
  create = /* @__PURE__ */contractPublisher.buildTransactionFunction(async metadataWithRewards => {
   ...
   * ```
   */
  addPackContents = /* @__PURE__ */contractPublisher.buildTransactionFunction(async (packId, packContents) => {
    const signerAddress = await this.contractWrapper.getSignerAddress();
   ...
   * ```
   */
  createTo = /* @__PURE__ */contractPublisher.buildTransactionFunction(async (to, metadataWithRewards) => {
    const uri = await contractPublisher.uploadOrExtractURI(metadataWithRewards.packMetadata, this.storage);
    ...
    return contractPublisher.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "createPack",
      args: [contents, numOfRewardUnits, uri, parsedMetadata.openStartTime, parsedMetadata.rewardsPerPack, await contractPublisher.resolveAddress(to)],
      parse: receipt => {
        ...
        parse: async receipt => {
          ...
          for (const reward of rewards) {
            switch (reward.tokenType) {
              case 0:
                {
                  const tokenMetadata = await contractPublisher.fetchCurrencyMetadata(_this.contractWrapper.getProvider(), reward.assetContract);
                  erc20Rewards.push({
                    contractAddress: reward.assetContract,
                    quantityPerReward: ethers.utils.formatUnits(reward.totalAmount, tokenMetadata.decimals).toString()
                  });
  ...
  async toPackContentArgs(metadataWithRewards) {
    ...
    for (const erc20 of erc20Rewards) {
      const normalizedQuantity = await contractPublisher.normalizePriceValue(provider, erc20.quantityPerReward, erc20.contractAddress);
  ...
  detectVrf() {
    if (contractPublisher.detectContractFeature(this.contractWrapper, "PackVRF")) {
