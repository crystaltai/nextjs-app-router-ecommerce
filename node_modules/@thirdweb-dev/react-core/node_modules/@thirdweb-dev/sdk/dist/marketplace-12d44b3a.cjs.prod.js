...
var marketplaceAuction = require('./marketplace-auction-dfd247ee.cjs.prod.js');
...
class Marketplace {
  ...
  constructor(network, address, storage) {
    ...
    this.roles = new contractPublisher.ContractRoles(this.contractWrapper, Marketplace.contractRoles);
  ...
  async isRestrictedToListerRoleOnly() {
    const anyoneCanList = await this.contractWrapper.readContract.hasRole(contractPublisher.getRoleHash("lister"), ethers.constants.AddressZero);
  ...
  async getOffers(listingId) {
    ...
    return await Promise.all(listingEvents.map(async e => {
      return await contractPublisher.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), {
   ...
   * ```
   */
  makeOffer = /* @__PURE__ */contractPublisher.buildTransactionFunction(async (listingId, pricePerToken, quantity) => {
    const listing = await this.contractWrapper.readContract.listings(listingId);
    if (listing.listingId.toString() !== listingId.toString()) {
      throw new contractPublisher.ListingNotFoundError(this.getAddress(), listingId.toString());
    }
      ...
      case marketplaceAuction.ListingType.Direct:
        {
          ...
          return await this.direct.makeOffer.prepare(listingId, quantity, contractPublisher.isNativeToken(listing.currency) ? contractPublisher.NATIVE_TOKENS[chainId].wrapped.address : listing.currency, pricePerToken);
        }
      case marketplaceAuction.ListingType.Auction:
        {
   ...
   */
  setBidBufferBps = /* @__PURE__ */contractPublisher.buildTransactionFunction(async bufferBps => {
   ...
   */
  setTimeBufferInSeconds = /* @__PURE__ */contractPublisher.buildTransactionFunction(async bufferInSeconds => {
    await this.roles.verify(["admin"], await this.contractWrapper.getSignerAddress());
   ...
   */
  allowListingFromSpecificAssetOnly = /* @__PURE__ */contractPublisher.buildTransactionFunction(async contractAddress => {
    const encoded = [];
    const members = await this.roles.get("asset");
    if (members.includes(ethers.constants.AddressZero)) {
      encoded.push(this.encoder.encode("revokeRole", [contractPublisher.getRoleHash("asset"), ethers.constants.AddressZero]));
    }
    encoded.push(this.encoder.encode("grantRole", [contractPublisher.getRoleHash("asset"), contractAddress]));
    return contractPublisher.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "multicall",
      args: [encoded]
   ...
   */
  allowListingFromAnyAsset = /* @__PURE__ */contractPublisher.buildTransactionFunction(async () => {
    const encoded = [];
    const members = await this.roles.get("asset");
    for (const addr in members) {
      encoded.push(this.encoder.encode("revokeRole", [contractPublisher.getRoleHash("asset"), addr]));
    }
    encoded.push(this.encoder.encode("grantRole", [contractPublisher.getRoleHash("asset"), ethers.constants.AddressZero]));
    return contractPublisher.Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "multicall",
      args: [encoded]
  ...
  async getAllListingsNoFilter(filterInvalidListings) {
    const listings = await Promise.all(Array.from(Array((await this.contractWrapper.readContract.totalListings()).toNumber()).keys()).map(async i => {
      ...
      try {
        ...
        if (err instanceof contractPublisher.ListingNotFoundError) {
          return undefined;
        } else {
